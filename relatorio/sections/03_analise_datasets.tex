...
\begin{enumerate}
    \item \textbf{Tamanho do \textit{Dataset}:} O conjunto de treino é notavelmente reduzido (n=731). Esta escassez de dados impõe um risco significativo de \textit{overfitting} para LLMs submetidos a \textit{fine-tuning} completo.
...
Ambos os fatores justificam a adoção de uma abordagem de \textit{Parameter-Efficient Fine-Tuning} (PEFT), como o LoRA\cite{hu2021loralowrankadaptationlarge}, e a utilização de estratégias de mitigação de desbalanceamento, como a ponderação de classes na função de perda (\textit{weighted loss}).
...
Esta discrepância dificulta uma comparação direta e constitui um desafio metodológico importante. Aplicar diretamente o classificador $M_{MGTD}$ (treinado em documentos) a uma sentença, ou aplicar o $M_{SD}$ (treinado em sentenças) diretamente a um documento $D_{MGTD}$, levaria a uma perda de performance.

Para superar este desafio, a nossa metodologia (detalhada na Secção \ref{sec:metodo}) emprega uma \textbf{estratégia de agregação}, onde o classificador $M_{SD}$ é aplicado iterativamente às sentenças de um documento $D_{MGTD}$ para calcular um \textbf{Índice de Subjetividade} agregado.
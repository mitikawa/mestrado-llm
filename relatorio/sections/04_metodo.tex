...
Nessa tarefa, optou-se pela utilização do \textit{distilbert/distilbert-base-uncased} \cite{sanh2020distilbertdistilledversionbert}. O DistilBERT é uma versão destilada do BERT, que possui o número de parâmetros significamente reduzido e mantendo bastante de seu desempenho em relação ao modelo original em tarefas de compreensão de linguagem natural. Essa compactação é obtida por meio do processo de \textit{knowledge distillation}, em que um modelo estudado (teacher) transfere seu comportamento para um modelo estudado mais leve (student).
